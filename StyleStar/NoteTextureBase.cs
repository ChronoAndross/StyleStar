...
namespace StyleStar
{
    public abstract class NoteTextureBase
    {
        ...
        protected void SetVerts(double x1l, double x1r, double y1, double x2l, double x2r, double y2, int yRepetition = 1)
        {
            SetVerts((float)x1l, (float)x1r, (float)y1, (float)x2l, (float)x2r, (float)y2, z);
        }
        ...
        protected void SetVerts(float x1l, float x1r, float y1, float x2l, float x2r, float y2, float z, int yRepetition = 1)
        {
            if (verts == null)
            ...
            else
            {
                ...
                // Set repetitions
                verts[0].TextureCoordinate.Y = yRepetition;
                verts[2].TextureCoordinate.Y = yRepetition;
        ...
        {
            GenerateVerts(x1l, x1r, y1, x2l, x2r, y2, z, 1);
        }

        private void GenerateVerts(float x1l, float x1r, float y1, float x2l, float x2r, float y2, float z, float repetitions)
        {
            ...
            verts[2].TextureCoordinate = new Vector2(1, repetitions);

            verts[3].TextureCoordinate = verts[1].TextureCoordinate;
            verts[4].TextureCoordinate = new Vector2(1, 0);
            verts[5].TextureCoordinate = verts[2].TextureCoordinate;
